...
**AIRA Login PoC 1 Developer Source Code Generation Standard**

_Java 25 Login PoC | Code Generation Rules | Security Controls | Evidence-First Implementation_

**v1.1 - Canonical 23A Numbering and Java 25 Login PoC Baseline Update**

| **Property** | **Value** |
| --- | --- |
| **Document ID** | **AIRA-DOC-023A** |
| **Document Title** | **AIRA Login PoC 1 Developer Source Code Generation Standard** |
| **Document Version** | **v1.1 - Canonical 23A Numbering and Java 25 Login PoC Baseline Update** |
| **Supersedes** | **23-AIRA_Login_PoC1_Developer_Source_Code_Generation_Standard_v1.0** |
...
| **Numbering Treatment** | **Login PoC implementation companion** |
...
**Document role. **Defines controlled source-code generation rules for the Login PoC 1 implementation under AIRA architecture and security standards.
...
Renumbered from Document 23 to 23A to avoid collision with Pack 04 Architecture Fitness Function Catalog.

Updated all backend implementation assumptions to Java 25 LTS and Spring Boot 4.x compatibility where feasible.

Aligned login source-generation rules with Pack 04 MicroFunction login patterns, Pack 05 Security Standard, and Pack 05 Flyway guide.

Reinforced that generated code must not create a custom login system; it assembles identity, session, policy, audit, event, and observability controls.

Added explicit GitNexus impact-analysis and CI evidence expectations for generated code before merge.
...
1. **Prompt A — Greenfield Implementation Prompt** for Kiel, Krisha, and Christian.

2. **Prompt B — Refactor / Continuation Prompt** for Henry, who will start from his existing working Login implementation.
...
| 04-AIRA_Technology_Stack_v9.1 | approved tools, frameworks, and runtime choices |
...
1. **Attributability** — source code, decisions, configuration, tests, and generated artifacts must be traceable to developer, prompt, branch, commit, transaction, and standard.

2. **Verifiability** — each output must have tests, audit records, policy decision evidence, local run instructions, and reproducibility evidence.

3. **Classifiability** — code, configuration, logs, prompts, policies, session context, and runtime data must carry appropriate classification handling.

4. **Improvability** — gaps, backlog items, refactoring opportunities, test coverage gaps, and future automation candidates must be documented.
