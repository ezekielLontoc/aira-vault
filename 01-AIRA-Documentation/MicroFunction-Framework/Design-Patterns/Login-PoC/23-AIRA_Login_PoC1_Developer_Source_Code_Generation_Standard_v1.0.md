...
source_system: "Desktop DOCX and Markdown Import"
source_docx_path: "C:\Users\krisha.yambao\Desktop\AIRA_AI Native v2.0\23-AIRA_Login_PoC1_Developer_Source_Code_Generation_Standard_v1.0.docx"
source_docx_hash: "94E2291E424EFB426499400432F8C7B4DB20AFC18D5EAF576EAA33625B6CE57F"
source_markdown_path: "C:\Users\krisha.yambao\Desktop\AIRA_AI Native v2.0\23-AIRA_Login_PoC1_Developer_Source_Code_Generation_Standard_v1.0.md"
source_markdown_hash: "55283401341393DABDF037C3F0086F94875C082D16BDCEE152D9E67CA09E17E0"
...
approval_status: "approved-for-team-execution"
tags:
  - AIRA
  - MicroFunction
  - Login
...
1. **Prompt A â€” Greenfield Implementation Prompt** for Kiel, Krisha, and Christian.
2. **Prompt B â€” Refactor / Continuation Prompt** for Henry, who will start from his existing working Login implementation.
...
|---|---|
| `01-AIRA_AVCI_Engineering_Standard_v3.0` | AVCI compliance, evidence, traceability, classification, improvement loop |
| `02-AIRA_Engineering_Blueprint_v5.0` | architecture baseline and engineering principles |
| `03-AIRA_Developer_Guide_v4.0` | developer execution standard |
| `04-AIRA_Technology_Stack_v8.0` or latest approved version | approved tools, frameworks, and runtime choices |
| `08-AIRA_Unit_Testing_Standard_v3.0` | deterministic testing and evidence requirements |
| `10-AIRA_MicroFunction_Framework_v3.0` | MicroFunction framework rules |
| `10A-AIRA_MicroFunction_Design_and_Development_Guide_v2.0` | MicroFunction design method |
| `10B-AIRA_MicroFunction_Framework_Implementation_Standard_v2.0` | runtime implementation rules |
| `10C-AIRA_MicroFunction_Sequence_Diagrams_and_Mermaid_Reference_v2.0` | sequence and design documentation support |
| `10D-AIRA_MicroFunction_Standard_Function_Catalog_and_Assembly_Templates_v2.0` | standard step catalog and assembly templates |
| `11-AIRA_AI_Native_DevSecOps_Standard_v3.0` | CI/CD, security gates, evidence pack |
| `12-AIRA_CLAUDE_MD_Rollout_Starter_Pack_v3.0` | AI assistant behavior and prompt governance |
| `15-AIRA_API_and_Integration_Contract_Standard_v2.0` | API contract, integration boundaries, safe response model |
| `16-AIRA_Database_Migration_and_Data_Engineering_Standard_v2.0` | Flyway, PostgreSQL, migration governance |
| `17-AIRA_Security_Identity_Secrets_and_Access_Control_Standard_v2.0` | identity, secrets, access control, least privilege |
| `18-AIRA_Repository_Bootstrap_and_Golden_Source_Implementation_Guide_v1.0` | repository structure and golden source discipline |
...
1. **Attributability** â€” source code, decisions, configuration, tests, and generated artifacts must be traceable to developer, prompt, branch, commit, transaction, and standard.
2. **Verifiability** â€” each output must have tests, audit records, policy decision evidence, local run instructions, and reproducibility evidence.
3. **Classifiability** â€” code, configuration, logs, prompts, policies, session context, and runtime data must carry appropriate classification handling.
4. **Improvability** â€” gaps, backlog items, refactoring opportunities, test coverage gaps, and future automation candidates must be documented.

## 6. Team Execution Model

| Developer | Starting Point | Required Prompt |
|---|---|---|
| Kiel | Greenfield / empty repository | Prompt A â€” Greenfield Implementation Prompt |
| Krisha | Greenfield / empty repository | Prompt A â€” Greenfield Implementation Prompt |
| Christian | Greenfield / empty repository | Prompt A â€” Greenfield Implementation Prompt |
| Henry | Existing working Login function | Prompt B â€” Refactor / Continuation Prompt |
...
2. Do not store, hash, compare, or validate passwords inside AIRA application code.
3. Authentication must be delegated to Keycloak and the enterprise AD target pattern.
4. AIRA application code must only establish the governed session context after identity, token, policy, classification, idempotency, audit, and observability controls pass.
5. Controllers must remain thin adapters.
6. Do not hardcode authorization logic in controllers, services, frontend code, or database scripts.
7. Authorization must use OPA/Rego policy with RBAC/ABAC/SBAC inputs.
8. Do not hardcode business roles or skills in application logic.
9. Do not expose raw JWT, refresh token, ID token, password, PII, or secrets in logs, UI, traces, tests, screenshots, or documentation.
10. Do not place secrets in source code, `.env`, screenshots, logs, tests, or documentation.
11. Use synthetic users and synthetic test data only.
12. Fail safe, not fail open.
13. If Keycloak, OPA, Vault, JWT validation, audit, runtime configuration, or required security dependency is unavailable, protected access must be denied safely.
14. MicroFunctions must be small, single-responsibility, testable, dependency-inverted, observable, reversible, and governed by runtime configuration.
15. Auto-heal, auto-learn, and auto-improve hooks must never weaken SOLID, Clean Architecture, testability, security, observability, reversibility, or AVCI evidence.
...
## 10. Standard Prompt A â€” Greenfield Implementation Prompt
...
- 04-AIRA_Technology_Stack_v8.0 or latest approved version
...
â”œâ”€â”€ CODEOWNERS
â””â”€â”€ .github/workflows/aira-ci.yml
```
...
## 11. Standard Prompt B â€” Refactor / Continuation Prompt
...
### Phase 2 â€” Separate Architecture Boundaries
...
### Phase 3 â€” Introduce MicroFunction Runtime
...
### Phase 4 â€” Externalize Configuration
...
### Phase 6 â€” Improve Frontend Safely
...
### Phase 7 â€” Add Evidence and Tests
...
3. Runtime transaction evidence
4. MicroFunction step coverage
5. Security and identity evidence
6. OPA policy test result
7. Backend test result
8. Frontend test result
9. CI/CD evidence if available
10. Known limitations
11. Rollback plan
12. AVCI compliance summary
13. SOLID and Enterprise Design Principles compliance summary
...
The teamâ€™s outputs shall be evaluated using the rubric below.
...
1. PoC 1 may use local Keycloak users for demonstration, provided the target enterprise AD federation pattern is documented.
2. Vault may use a safe dev-mode adapter or stub in local development, provided production secrets are not introduced.
3. Kafka may be represented by a transactional outbox table first. Event broker publishing may be added in a future iteration.
4. Redis or Valkey may be used for acceleration, but PostgreSQL remains authoritative where persistence and audit evidence are required.
5. The implementation must use synthetic users and test data only.
6. Any generated code is subject to human review and must not be promoted automatically.
