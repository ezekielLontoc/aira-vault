---
document_id: "AIRA-DOC-002"
title: "AIRA Engineering Blueprint"
version: "v5.1"
status: "aligned"
source_file: "02-AIRA_Engineering_Blueprint_v5.1_Aligned.docx"
source_format: "docx"
...
# AIRA Engineering Blueprint

**AIRA**

AI-Native Enterprise Platform

**AIRA AI-Native Engineering Blueprint**

_Build-Ready System Specification for the Software Development Team_

**v5.1 - Cross-Pack Architecture Synchronization Update**

| **Property** | **Value** |
| --- | --- |
| **Document ID** | **AIRA-DOC-002** |
| **Document Title** | **AIRA AI-Native Engineering Blueprint** |
| **Document Version** | **v5.1 - Cross-Pack Architecture Synchronization Update** |
| **Supersedes** | **02-AIRA_Engineering_Blueprint_v5.0** |
...
**Document role. **Build-ready architecture authority for service boundaries, AI execution architecture, workflow partitioning, and evidence flows.
...
Aligned architecture authority with the current Technology Stack v9.0, including Dograh voice orchestration and GitNexus read-only code intelligence.

Resolved the Pack 01 01A gap by referencing the new canonical 01A v1.1 document rather than the former Document 28 naming.

Clarified that Temporal owns machine-durable workflows and compensation, while Flowable owns human approvals, BPMN/DMN, CAB, and exception review.

Reinforced LiteLLM-only model routing, NeMo Input/Retrieval/Execution/Output guardrails, and Harness/SBAC/OPA-mediated agent actions.

Aligned database and schema evolution language with Pack 05 Flyway-from-Day-0 governance.
...
| --- | --- |
| 01A canonical reference | Introduces 01A-AIRA Enterprise Design Principles and SOLID Enforcement Layer v1.1 as the canonical cross-document principle source. |
...
| L1 | 02-AIRA Engineering Blueprint v5.1 | Build-ready system specification: boundaries, flows, architecture controls, and evidence model. |
...
| flowchart TD<br> UI[Presentation Layer] --> GW[Enterprise Edge / Spring Cloud Gateway]<br> GW --> APP[Application and Domain Services]<br> APP --> WF[Workflow and Process: Temporal + Flowable + MicroFunctions]<br> APP --> AI[AI Orchestration and Safety: Hermes + Guardrails + LiteLLM + Harness]<br> APP --> DATA[Data, Knowledge and Retrieval: OpenKM + PostgreSQL + pgvector + LightRAG + LLM Wiki]<br> APP --> EVT[Integration and Eventing: Kafka + Avro + CloudEvents]<br> AI --> POL[OPA + SBAC + Trust + Audit]<br> WF --> AUD[Evidence and Audit]<br> DATA --> AUD<br> EVT --> AUD |
| --- |
...
| sequenceDiagram<br> autonumber<br> actor User<br> participant GW as Gateway<br> participant API as Transaction API<br> participant MF as MicroFunction Runtime<br> participant BUS as STP-BUS-* Function<br> participant DB as PostgreSQL<br> participant AUD as Audit Evidence<br> User->>GW: Submit request<br> GW->>API: Authenticate, classify, correlate<br> API->>MF: execute(txn_code, payload, actor, tenant, idempotency_key)<br> MF->>MF: Standard controls: security, parse, normalize, validate, idempotency, lock<br> MF->>BUS: Execute isolated business capability<br> BUS->>DB: Persist through approved port/repository<br> MF->>AUD: Write step evidence and safe response metadata<br> MF-->>API: Success or safe error response |
| --- |
...
| flowchart TD<br> H[Human Request or System Signal] --> R[Agent Reasoning]<br> R --> G1[Input and Retrieval Guardrails]<br> G1 --> P[Proposed Action with Evidence, Risk, Confidence]<br> P --> G2[Execution Guardrail]<br> G2 --> HAR[Harness]<br> HAR --> SBAC[Skill and Trust Check]<br> SBAC --> OPA[OPA Policy Decision]<br> OPA -->\|Low Risk Approved\| TOOL[Approved Tool Action]<br> OPA -->\|High Risk or Low Confidence\| FLOW[Flowable Human Approval]<br> FLOW --> TOOL<br> TOOL --> AUD[Immutable Audit Evidence]<br> AUD --> LEARN[Governed Feedback Loop] |
| --- |

| **Rejected AI Patterns**<br>Direct provider SDK calls and direct OpenRouter calls, ungoverned OpenRouter calls, bypass flags for guardrails, direct agent credentials, autonomous production execution, unreviewed prompt/guardrail edits, real PII in prompts/tests, and self-modifying AI artifacts are prohibited architecture patterns. |
| --- |
...
| ## AIRA Architecture Compliance Summary<br><br>### Attributable<br>- Owner:<br>- Ticket / ADR / TDL:<br>- AI tool/model used, if any:<br><br>### Verifiable<br>- Tests run:<br>- Fitness functions passed:<br>- Security/policy checks:<br>- Contract/schema checks:<br><br>### Classifiable<br>- Data classification affected:<br>- Model route eligibility:<br>- Logging/redaction impact:<br><br>### Improvable<br>- Improvement rationale:<br>- Metrics affected:<br>- Follow-up backlog item, if any:<br><br>### Enterprise Design Principle Impact<br>- SOLID impact:<br>- Clean Architecture / DDD / Ports-Adapters impact:<br>- Idempotency / determinism / resilience impact:<br>- Observability / security / policy-as-code impact:<br>- Reversibility / rollbackability impact:<br>- Waiver required: Yes/No |
| --- |
...
| Classifiable | Document and architecture artifacts are INTERNAL CONFIDENTIAL; runtime artifacts inherit classification-driven routing and handling. |
