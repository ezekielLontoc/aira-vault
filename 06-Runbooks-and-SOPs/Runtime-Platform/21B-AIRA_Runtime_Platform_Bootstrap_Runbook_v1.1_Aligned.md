---
document_id: "AIRA-DOC-021B"
title: "AIRA Runtime Platform Bootstrap Runbook"
version: "v1.1"
status: "aligned"
source_file: "21B-AIRA_Runtime_Platform_Bootstrap_Runbook_v1.1_Aligned.docx"
source_format: "docx"
vault_folder: "06-Runbooks-and-SOPs/Runtime-Platform"
...
# AIRA Runtime Platform Bootstrap Runbook
...
_Platform Bootstrap | Kubernetes | GitOps | Secrets | Observability | Runtime Evidence_

**v1.1 - Canonical 21B Numbering and Pack 03 v1.2 Alignment Update**

| **Property** | **Value** |
| --- | --- |
| **Document ID** | **AIRA-DOC-021B** |
| **Document Title** | **AIRA Runtime Platform Bootstrap Runbook** |
| **Document Version** | **v1.1 - Canonical 21B Numbering and Pack 03 v1.2 Alignment Update** |
| **Supersedes** | **21-AIRA_Runtime_Platform_Bootstrap_Runbook_v1.0** |
...
| **Numbering Treatment** | **Platform bootstrap runbook** |
...
**Document role. **Operational runbook for bootstrapping the runtime platform and proving infrastructure readiness with evidence.
...
Renumbered from duplicate Document 21 to 21B to avoid collision with 21A knowledge-control-plane standard.

Aligned runtime bootstrap with Technology Stack v9.1 while preserving v9.0 source baseline lineage.

Added Java 25 service-template validation as a platform-readiness check for backend workloads.

Reinforced Argo CD/GitOps-only deployment, no manual production kubectl mutation, Vault/OpenBao secrets control, and OTel evidence.

Added explicit evidence for GitNexus, Dograh, LiteLLM, NeMo Guardrails, Harness/SBAC/OPA readiness where those components are piloted.
...
| Companion Documents | 01 AVCI v3.1; 01A Enterprise Design Principles; 02 Engineering Blueprint v5.1; 04 Technology Stack v9.1; 09 Greenfield Environment v3.0; 11 DevSecOps v3.1; 17 Security v2.0; 18 Repository Bootstrap; 20 CI/CD Evidence Pack |
...
| **Runtime Bootstrap Position**<br>The runtime platform is not a collection of independently installed tools. It is a governed platform product. Every component must have an owner, version, source, hardening baseline, configuration-as-code, health check, recovery path, and evidence record. |
| --- |
...
![[attachments/21B-AIRA_Runtime_Platform_Bootstrap_Runbook_v1.1_Aligned/image5.png]]

_Figure 1. Runtime bootstrap dependency sequence B0-B12._

![[attachments/21B-AIRA_Runtime_Platform_Bootstrap_Runbook_v1.1_Aligned/image6.png]]

_Figure 2. AIRA runtime platform layered view._

![[attachments/21B-AIRA_Runtime_Platform_Bootstrap_Runbook_v1.1_Aligned/image7.png]]

_Figure 3. Governed runtime contribution, deployment, and evidence path._

![[attachments/21B-AIRA_Runtime_Platform_Bootstrap_Runbook_v1.1_Aligned/image8.png]]

_Figure 4. Ready-to-Develop validation gate with fail-closed behavior._
...
| **Exit Evidence**<br>Approved bootstrap ticket, version baseline, RACI, evidence path, risk register. |
| --- |
...
| **Exit Evidence**<br>Host inventory, hardening report, package manifest, NTP validation, admin access test. |
| --- |
...
| **Exit Evidence**<br>Network map, DNS records, time sync evidence, CA trust validation, firewall rule export. |
| --- |
...
| **Exit Evidence**<br>Cluster health report, namespace/RBAC manifest, network policy test, admission control result. |
| --- |
...
| **Exit Evidence**<br>Registry project list, immutability policy, scan report, signed artifact test, pull policy evidence. |
| --- |
...
| **Exit Evidence**<br>OIDC test, MFA/group mapping, Vault audit enabled, dynamic secret test, mTLS identity test, OPA allow/deny tests. |
| --- |
...
| **Exit Evidence**<br>DB migration smoke test, backup test, pgvector test, Kafka produce/consume test, schema test, cache loss test. |
| --- |
...
| **Exit Evidence**<br>Temporal workflow execution, compensation test, Flowable user task, DMN decision test, audit evidence. |
| --- |
...
| **Exit Evidence**<br>Dashboard screenshots, telemetry sample, alert route test, trace/log/metric correlation test, retention settings. |
| --- |
...
| **Exit Evidence**<br>Model alias test, guardrail block/pass test, Harness authorization test, OPA policy test, audit record. |
| --- |
...
| **Exit Evidence**<br>Document upload smoke test, metadata record, retrieval evidence pack, classification route test, no real PII confirmation. |
| --- |
...
| **Exit Evidence**<br>Runner registration, sample pipeline run, Argo sync result, signed image deployed, evidence manifest. |
| --- |
...
| **Exit Evidence**<br>Sample transaction evidence, step execution audit, outbox event, trace dashboard, test report. |
| --- |
...
| **Waiver Rule**<br>A waiver may not approve fail-open identity, secrets, OPA, audit, guardrail, classification, or direct model-provider bypass. Waivers must include owner, expiry, risk acceptance, compensating control, and remediation backlog item. |
| --- |
...
| bootstrap_evidence_manifest:<br> document_id: AIRA-DOC-021<br> bootstrap_run_id: BR-YYYYMMDD-001<br> environment: DEV-FOUNDATION<br> classification: INTERNAL<br> owner: platform-engineering<br> approved_by: solutions-architect<br> source_references:<br> git_commit: <commit-sha><br> technology_stack_version: 04-AIRA_Technology_Stack_v9.1<br> security_standard_version: 17-AIRA_Security_Identity_Secrets_and_Access_Control_Standard_v2.0<br> components:<br> - name: kubernetes<br> version: <pinned-version><br> source: golden-source/helm/kubernetes-baseline<br> evidence_ref: OpenKM:/AIRA/Platform/B3/k8s-health.pdf<br> status: PASS<br> - name: vault<br> version: <pinned-version><br> source: golden-source/helm/vault<br> evidence_ref: OpenKM:/AIRA/Platform/B5/vault-policy-test.pdf<br> status: PASS<br> readiness_decision:<br> status: APPROVED \| CONDITIONAL \| REJECTED<br> residual_risks: []<br> approver: <name><br> approval_date: <yyyy-mm-dd> |
| --- |
...
| flowchart TD<br> B0[Approval and Baseline Freeze] --> B1[Proxmox / Linux Foundation]<br> B1 --> B2[Network / DNS / NTP / Private CA]<br> B2 --> B3[Kubernetes Base]<br> B3 --> B4[Golden Source / Harbor]<br> B4 --> B5[Identity / Secrets / Workload Identity / OPA]<br> B5 --> B6[PostgreSQL / Kafka / Redis]<br> B6 --> B7[Temporal / Flowable]<br> B7 --> B8[Observability / Audit / Evidence]<br> B8 --> B9[LiteLLM / NeMo Guardrails / Harness / SBAC]<br> B9 --> B10[OpenKM / LLM Wiki / pgvector / LightRAG]<br> B10 --> B11[GitOps / CI Runners]<br> B11 --> B12[Ready-to-Develop Gate] |
| --- |

Mermaid D2. Governed runtime contribution and deployment path.

| flowchart LR<br> DEV[Developer / AI Assistant] --> GIT[Protected Git Repository]<br> GIT --> CI[CI Pipeline: Tests, Scans, Evidence]<br> CI --> REG[Golden Source Registry]<br> REG --> ARGO[Argo CD GitOps]<br> ARGO --> OPA[Admission / OPA Policies]<br> OPA --> K8S[Kubernetes Runtime]<br> K8S --> OBS[Observability + Audit Evidence]<br> OBS --> LEARN[Governed Auto-Learn Feedback] |
| --- |
